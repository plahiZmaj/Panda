...
def main():
    ...
    def execute_motion(goto_pose):
        group.set_pose_target(goto_pose)
        test_plan = group.plan()
        group.execute(test_plan, wait=True)
        if sick.state:
            rospy.logwarn("Motion stopped due to safety")
            while sick.state:
                rate.sleep()
            rospy.logwarn("Resuming motion")
            execute_motion(goto_pose)
        return
    ...
    while True:
        ...
        #test_plan = group.plan()
        execute_motion(get_pose_from_tf("Home",tf_buffer))
        #group.execute(test_plan, wait=True)
        #while sick.state:
        #    pass
        group.stop() 
